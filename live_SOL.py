...
from strategy_engine_SOL import Entry, StrategyParams, StrategyState
...
def create_strategy_state() -> StrategyState:
    params = StrategyParams(
        ...
        tp2_pct=float(os.getenv("TP2_PCT", "0.15")),
        sl_pct=float(os.getenv("SL_PCT", "0.18")),
        tp1_sell_prop=float(os.getenv("TP1_SELL_PROP", "0.4")),
    )
...
def create_exchange() -> ccxt.Exchange:
    ...
    try:
        if HEDGE_MODE:
            ex.set_leverage(LEVERAGE, SYMBOL, {"mgnMode": TD_MODE, "posSide": "long"})
            ex.set_leverage(LEVERAGE, SYMBOL, {"mgnMode": TD_MODE, "posSide": "short"})
        else:
            ex.set_leverage(LEVERAGE, SYMBOL, {"mgnMode": TD_MODE})
    except Exception:
        pass
    return ex
...
def _infer_tp1_done_from_trades(
    ...
    for t in relevant:
        ...
        sol = amount * contract_size
        if sol <= 0:
            continue

        if pos_side == "long":
            delta = sol if side == "buy" else -sol
        else:
            delta = sol if side == "sell" else -sol
...
def sync_state_from_exchange(exchange: ccxt.Exchange, state: StrategyState) -> None:
    ...
    for p in positions:
        ...
        if contracts <= 0:
            continue
        sol_size = contracts * contract_size
        ...
        if side == "long":
            ...
            long_entry = Entry(price=entry_price, size=sol_size, tp1_done=tp1_done)
        elif side == "short":
            tp1_done = _infer_tp1_done_from_trades(trades, contract_size, "short") if HEDGE_MODE else False
            short_entry = Entry(price=entry_price, size=sol_size, tp1_done=tp1_done)
...
def execute_actions(exchange: ccxt.Exchange, actions: list[dict]) -> list[dict]:
    ...
    for act in actions:
        ...
        if sol_size <= 0:
            continue
        sol_size = float(exchange.amount_to_precision(SYMBOL, sol_size))
        if sol_size <= 0:
            continue
        order = exchange.create_order(SYMBOL, "market", side, sol_size, None, params)
        executed.append({"action": {**act, "contracts": sol_size}, "order": order})
    return executed
...
def print_summary(exchange: ccxt.Exchange, df: pd.DataFrame, state: StrategyState, executed: list[dict]) -> None:
    ...
    print(
        ...
        "(~", f"{long_size * contract_size:.6f}", "SOL )",
    )
    print(
        "Short contracts:", f"{short_size:.6f}",
        "(~", f"{short_size * contract_size:.6f}", "SOL )",
    )
